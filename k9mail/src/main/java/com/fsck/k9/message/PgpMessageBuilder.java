...
import com.fsck.k9.ui.crypto.InbomeOperations;
...
public class PgpMessageBuilder extends MessageBuilder {
    ...
    private void attachKeyInOpenPgpHeader() {
        String inbomeAddress = currentProcessedMimeMessage.getFrom()[0].getAddress();
        ...
        gimmeKeyIntent.putExtra(OpenPgpApi.EXTRA_MINIMIZE_USER_ID, inbomeAddress);
        ...
        if (result.getIntExtra(OpenPgpApi.RESULT_CODE, OpenPgpApi.RESULT_CODE_ERROR) == OpenPgpApi.RESULT_CODE_SUCCESS) {
            ...
            String inbomeHeader = InbomeOperations.INBOME_HEADER + ": ";
            inbomeHeader += InbomeOperations.INBOME_PARAM_TO + "=" + inbomeAddress + ";";
            inbomeHeader += InbomeOperations.INBOME_PARAM_KEY_DATA + "=" + ByteString.of(keyMaterial).base64();
            StringBuilder headerLines = new StringBuilder();
            for (int i = 0, j = inbomeHeader.length(); i < j; i += 76) {
                if (i +76 > j) {
                    headerLines.append(inbomeHeader.substring(i)).append("\n ");
                } else {
                    headerLines.append(inbomeHeader.substring(i, i+76)).append("\n ");
                }
            }

            String rawHeader = headerLines.toString();
            Log.d(K9.LOG_TAG, rawHeader);
            currentProcessedMimeMessage.addRawHeader("Inbome", rawHeader);
        }
    }
